...
def gen_dataset(filename, split):
    with open(filename, "r") as fh:
        data = json.load(fh)
    name2label = {}
    for docid, doc in tqdm(enumerate(data), desc=f"Reading DocRED {split} dataset", total=len(data), ncols=80):
        title: str = doc['title']
...
def gen_dataset_graphs(filename, split, name):
    ...
    graphs = []
    for docid, doc in tqdm(enumerate(dataset), desc=f"gen {name} dataset graph", ncols=100, total=len(dataset)):
    ...
    json.dump(graphs, open(f"./dataset/{name}_graph.json", "w"))
    return graphs
